...
export default function Footer() {
  return (
    <footer
      ...
      style={{
        background: 'linear-gradient(135deg, #F5C842 0%, #22C55E 50%, #14532d 80%, #060A06 100%)',
      }}
    >
      <div className="max-w-[1280px] mx-auto">
        <div className="bg-[#060A06] border border-[#182B18] rounded-3xl px-8 py-10 md:px-12 md:py-12 shadow-2xl">
          <div className="grid grid-cols-2 gap-8 md:grid-cols-3 lg:grid-cols-5">
            ...
            <div className="col-span-2 md:col-span-3 lg:col-span-1">
              ...
              <p className="text-xs text-[#728A72] leading-relaxed mb-4">
                Strategic VA campaigns for electronic music artists.
              </p>
              <span className="inline-flex items-center gap-2 text-xs text-[#728A72] border border-[#182B18] rounded-full px-3 py-1.5">
                <span>🌍</span>
            ...
            {footerColumns.map((col) => (
              <div key={col.title}>
                <h4 className="text-sm font-bold text-[#22C55E] mb-4 tracking-tight uppercase">{col.title}</h4>
                <ul className="flex flex-col gap-2.5">
                  {col.links.map((link) => (
                    <li key={link.label}>
                      <a
                        href={link.href}
                        className="text-sm text-[#728A72] hover:text-[#F0EDE6] transition-colors"
                      >
                        {link.label}
                      </a>
                    </li>
          ...
          <div className="mt-16 pt-6 border-t border-[#182B18] flex flex-col md:flex-row items-center justify-between gap-4">
            <p className="text-sm text-[#728A72]">© 2026 Mustache Crew Records. All rights reserved.</p>
            <div className="flex items-center gap-4">
              <a href="#" aria-label="Instagram" className="text-[#728A72] hover:text-[#22C55E] transition-colors">
              ...
              <a href="#" aria-label="SoundCloud" className="text-[#728A72] hover:text-[#22C55E] transition-colors">
              ...
              <a href="#" aria-label="Beatport" className="text-[#728A72] hover:text-[#22C55E] transition-colors">
