              ...
              <div key={i} className="flex gap-6 shrink-0">
                {images.map((image, index) => (
                  <div
                    ...
                    className="shrink-0 rounded-2xl overflow-hidden border-2 border-[#1E3A5F] bg-[#13294A] shadow-[0_20px_50px_-15px_rgba(0,0,0,0.6)] transition-transform duration-300 hover:scale-105 cursor-pointer"
                  >
          ...
          {images.map((image, index) => {
            ...
            return (
              ...
              >
                <div
                  style={{ width: cardSize, height: cardSize }}
                  className="rounded-2xl overflow-hidden border-2 border-[#1E3A5F] bg-[#13294A] shadow-[0_20px_50px_-15px_rgba(0,0,0,0.6)]"
                >
                  <img src={image.src} alt={image.alt} className="w-full h-full object-cover" loading="lazy" />
                </div>
              </motion.div>
          ...
          {Array(marqueeRepeat).fill(0).map((_, i) => (
            <div key={i} className="flex gap-6 shrink-0">
              {images.map((image, index) => (
                <div
                  key={`${i}-${index}`}
                  className="w-[200px] h-[200px] rounded-xl overflow-hidden border border-[#1E3A5F] bg-[#13294A] shrink-0"
                  onClick={() => onImageClick?.(index)}
                >
                  <img src={image.src} alt={image.alt} className="w-full h-full object-cover" loading="lazy" />
                </div>
